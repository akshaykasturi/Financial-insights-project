...
export default function ChatPage() {
  ...
  async function sendMessage(text) {
    ...
    const wakeupTimer = setTimeout(() => setShowWakeupHint(true), 8000);
        ...
        {isEmpty ? (
          <div className="flex-1 flex flex-col items-center justify-center text-center px-4">
            ...
            <button
              ...
              className="mt-8 text-xs text-slate-light hover:text-brass transition-colors underline underline-offset-2"
            >
              {showAbout ? "Hide" : "About this data"}
            </button>

            {showAbout && (
              <div className="mt-4 max-w-lg text-left text-xs text-slate-light leading-relaxed bg-ink-card border hairline rounded-lg p-4 space-y-2">
                <p>
                  Built on 28 years of Nifty 50 historical data (1998–2026), processed through
                  a Bronze → Silver → Gold pipeline on Databricks.
                </p>
                <p>
                  Five specialized AI agents handle ingestion, analysis, and reporting,
                  coordinated by an orchestrator. This chat is powered by retrieval-augmented
                  generation over real aggregated market statistics — every figure is sourced
                  from the underlying dataset, not generated freehand.
                </p>
              </div>
            )}
          </div>
          ...
          >
            ...
            {loading && <TypingIndicator showWakeupHint={showWakeupHint} />}
          </div>
        )}
...
function TypingIndicator({ showWakeupHint }) {
  return (
    <div className="flex flex-col gap-2">
      ...
      {showWakeupHint && (
        <p className="text-xs text-slate-light pl-1">
          Waking up the insights engine — this can take up to a minute on first load.
        </p>
      )}
    </div>
  );
